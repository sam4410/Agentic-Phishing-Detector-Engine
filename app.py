...
def render_step_timeline(container, current_step: int):
    steps = [
        "Input Received",
        "URL Analysis",
        "Content Analysis",
...
AGENT_INFO = {
    'url_analyzer': {
        ...
        'icon': '🔍',
        'color': '#3b82f6',
        'description': 'Analyzes URL structure, domain reputation, and detects typosquatting',
        'capabilities': [
    ...
    'content_analyzer': {
        ...
        'icon': '📝',
        'color': '#8b5cf6',
        'description': 'Examines text for social engineering tactics and phishing keywords',
        'capabilities': [
            '✓ Keyword analysis',
            '✓ Urgency detection',
    ...
    'visual_analyzer': {
        ...
        'icon': '👁️',
        'color': '#ec4899',
        'description': 'Inspects HTML structure and identifies brand impersonation',
        'capabilities': [
    ...
    'threat_intel': {
        ...
        'icon': '🛰️',
        'color': '#f97316',
        'description': 'Validates URLs and domains against external threat intelligence feeds',
        'capabilities': [
            '✓ VirusTotal reputation checks',
            '✓ Known phishing feed correlation',
            '✓ Malware & fraud classification',
            '✓ Detection count analysis',
            '✓ External verdict enforcement'
        ]
    },
    'coordinator': {
        ...
        'icon': '🧠',
        'color': '#10b981',
        'description': 'Synthesizes all findings and provides final threat assessment',
        'capabilities': [
...
def create_workflow_visualization(current_step=0):
    """Create an interactive workflow diagram (SOC-aligned)"""
...
def create_risk_gauge(value, title, ti_confirmed=False):
    """Create a SOC-aligned gauge chart for risk visualization"""

    # Dynamic reference for delta
    delta_reference = 30 if value < 60 else 60

    bar_color = "#dc2626" if ti_confirmed else "darkblue"
    threshold_color = "red" if ti_confirmed else "#ef4444"
...
def create_findings_chart(findings_by_category):
    ...
    for agent in AGENT_ORDER:
        if agent in findings_by_category:
            ...
            counts.append(findings_by_category[agent])
...
with tab1:
    ...
    if analyze_button and user_input:
        ...
        render_step_timeline(timeline_placeholder, 0)
        ...
        if input_type == "URL":
            ...
            timeline_placeholder.empty()
            render_step_timeline(timeline_placeholder, 1)
        ...
        timeline_placeholder.empty()
        render_step_timeline(timeline_placeholder, 2)
        ...
        if input_type == "URL":
            ...
            timeline_placeholder.empty()
            render_step_timeline(timeline_placeholder, 3)
        ...
        if input_type == "URL":
            ...
            timeline_placeholder.empty()
            render_step_timeline(timeline_placeholder, 4)
        ...
        timeline_placeholder.empty()
        render_step_timeline(timeline_placeholder, 5)
        ...
        render_step_timeline(timeline_placeholder, 6)
